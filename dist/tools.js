...
function requireNumber(args, key) {
    ...
    if (!Number.isFinite(val))
        throw new Error(`${key} must be a finite number, got ${args[key]}`);
    return val;
}
...
export function createStormTools(client) {
    return [
        // ── Read tools ───────────────────────────────────────────────────────
        {
            name: 'storm_get_strategy_info',
            description: 'Get strategy summary: TON balance in strategy, daily/weekly/monthly/yearly PnL, win rate, trades per week, max drawdown, low balance state. Call this FIRST to understand available capital before sizing positions.',
        ...
        },
        {
            ...
            description: 'Get portfolio: total realized and unrealized PnL in TON, list of tokens with quantities, PnL per token, entry prices, and current values.',
            parameters: { type: 'object', properties: {}, additionalProperties: false },
            handler: async () => {
                try {
        ...
        },
        {
            ...
            description: 'Get all open futures positions for the strategy. Returns a list of deals with direction (long/short), asset, leverage, margin, entry price, PnL, TP/SL prices, and swap status. Use this to see what positions are currently active before opening new ones.',
            parameters: { type: 'object', properties: {}, additionalProperties: false },
            handler: async () => {
                try {
        ...
        },
        {
            ...
            description: 'Get all deals including closed ones. Use this to review full trade history, calculate cumulative PnL, and learn from past trades. Closed deals have closed_at != null.',
            parameters: { type: 'object', properties: {}, additionalProperties: false },
            handler: async () => {
                try {
        ...
        },
        {
            ...
            description: 'Get TP/SL order history with pagination. Returns trigger prices, sizes, statuses, and directions.',
            parameters: {
                type: 'object',
                properties: {
            ...
            handler: async (args) => {
                try {
                    return await client.getOrders(args.limit ?? 20, args.offset ?? 0);
                }
                catch (err) {
                    return wrapError(err);
                }
        ...
        },
        {
            ...
            description: 'Check the status of an async operation after opening or closing a position. Statuses: "pending" = waiting for on-chain settlement, "confirmed" = done, "failed" = error (check failure_reason). Use the operation_id returned by storm_open_position or storm_close_position.',
            parameters: {
                type: 'object',
                properties: {
            ...
            handler: async (args) => {
                try {
                    ...
                    if (leverage < 2 || leverage > 100)
                        return { error: `leverage must be 2–100, got ${leverage}` };
                    return await client.openPosition({
                        ...
                        amount,
                        margin: amount,
                        deal_type: args.deal_type ?? 'market',
                        stop_loss_price: args.stop_loss_price ?? 0,
                        take_profit_price: args.take_profit_price ?? 0,
                        limit_price: args.limit_price ?? 0,
                        stop_trigger_price: args.stop_trigger_price ?? 0,
                        stop_price: 0,
                        entry_price: 0,
                        el_price: 0,
                    });
        ...
        },
        {
            ...
            description: 'Close an existing futures position (fully or partially) via /agent/execute. Provide the deal ID and amount in nanoTON. For full close, use the full margin amount of the position. Returns { operation_id, operation_status } — pass operation_id to storm_wait_confirmation.',
            parameters: {
                type: 'object',
                properties: {
        ...
        },
        {
            ...
            description: 'Create a Take Profit or Stop Loss order for an existing open position via /agent/execute. Call AFTER storm_wait_confirmation confirms the position is open. Specify order_type ("takeProfit" or "stopLoss"), base asset (e.g. "BTC" — extract from pair like "BTC/USD"), direction of the position, margin in nanoTON, order size in nanoTON, and trigger price in nanoTON. IMPORTANT: triggerPrice is in nanoTON, NOT USD.',
            parameters: {
                type: 'object',
                properties: {
        ...
        },
        {
            ...
            description: 'Wait for an operation to be confirmed on-chain. Polls status every 3 seconds until the operation reaches a terminal state (confirmed/failed/adjusted) or times out after 120 seconds. Use after storm_open_position or storm_close_position. If you prefer manual polling, use storm_get_operation_status instead.',
            parameters: {
                type: 'object',
                properties: {
